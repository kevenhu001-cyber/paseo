...
- **The loaded tail is bounded.** Layout, tool grouping, turn timing, and reducer passes are all O(tail) per commit, and nothing else shrinks the tail, so an uncapped transcript slows every commit the longer the app runs. `timeline/tail-retention.ts` evicts the oldest loaded prefix past `MAX_LOADED_TAIL_ITEMS`, advances `cursor.startSeq`, and flips `hasOlder` back on so a `before` fetch restores the rows on scroll-up. Mounted views report the lowest seq they render through `useStreamHistoryWindow`; eviction never crosses that floor, and rows without a timeline cursor are never evicted because they cannot be refetched. "before" merges are exempt — trimming them would undo the fetch that just landed.
